...
def SolicitaPuestoListRRHH(request):
    ...
    generalQuery = solicita_puesto.objects.filter(departamento__id=108).filter(
        estado_aprobacion=1).order_by('fecha_aprueba')
    if queryset:

        q1 = generalQuery.exclude(estado_ingreso=1).exclude(estado_ingreso=0)
        # q1 = solicita_puesto.objects.filter(activo_depar__id=108).filter(estado_aprobacion=1).order_by('fecha_aprueba')
...
def none(request):
    none = solicita_puesto.objects.filter(departamento__id=108).filter(
        estado_aprobacion=1).order_by('fecha_aprueba')
    context = {
        'nonelist': none
    }
...
def ajax_save_status(request):
    ...
    solicita_puesto.objects.filter(id=idn).update(
...
def ajax_save(request):
    ...
    solicita_puesto.objects.filter(id=idn).update(notasges=notas)
